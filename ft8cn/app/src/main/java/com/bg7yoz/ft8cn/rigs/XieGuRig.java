...
public class XieGuRig extends BaseRig {
    ...
    private void showAlert() {
        ...
        // Convert SWR from internal value to actual SWR (120 = 3.0)
        float actualSwr = swr >= 120 ? 3.0f : (swr / 40.0f);
        ...
        if ((alc < IcomRigConstant.xiegu_alc_alert_min)
            ...
            if (!alcMinAlert) {
                alcMinAlert = true;
                hasAlcWarning = true;
                ToastMessage.show(String.format(GeneralVariables.getStringFromResource(R.string.alc_low_alert), (float)alc));
            }
        } else {
            alcMinAlert = false;
        }
